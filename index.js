 ...
 * {
 *   success: true/false,
 *   results: { ... }    <-- results.say is the speech string
 * }
 *
 * ALSO duplicate top-level say/text for compatibility:
 * {
 *   success,
 *   say,
 *   text,
 ...
 * }
 *
 * IMPORTANT: do NOT create nested results inside results.
 * That is what causes results.results.say.
 */
...
function sendFail(res, message, extra = {}) {
  ...
  return res.status(200).json({
    ...
    say,
    text: say,
    results: { message: say, say, text: say, ...extra },
  });
}
...
app.all("/mb/schedule", async (req, res) => {
  ...
  try {
    ...
    // Support onlySay / only_say / onlysay / onlySAY
    const onlySayRaw =
    ...
    // Small payload to avoid tool size limits / parsing failures
    if (wantsOnlySay) {
      return sendSuccess(res, {
        date,
        timezone: STUDIO_TZ,
...
// 4) BOOK (existing client required in this endpoint)
