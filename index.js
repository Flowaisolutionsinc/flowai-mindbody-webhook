...
/**
 * ============
 * STABLE RESPONSE SHAPE (NEVER-BREAKS)
 * ============
 * We ALWAYS include say/text in BOTH places:
 *  - top-level say/text (supports platform paths like results.say)
 *  - nested results.say/results.text (supports platform paths like results.results.say)
 *
 * So regardless of wrapping, one of these exists:
 *  - results.say
 *  - results.results.say
 *  - say
 */
...
function sendFail(res, message, extra = {}) {
  const say = message || "Sorry — I couldn’t access that right now.";
  return res.status(200).json({
    success: false,
...
// 2) SCHEDULE (USE ?onlySay=1 in your Custom Action URL)
app.all("/mb/schedule", async (req, res) => {
  ...
  try {
    ...
    if (wantsOnlySay) {
      // SMALL PAYLOAD: prevents tool/voice platforms from choking
      return sendSuccess(res, {
        date,
        timezone: STUDIO_TZ,
        say,
...
// 4) BOOK (existing client id required here)
